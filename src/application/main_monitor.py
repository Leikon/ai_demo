...
from asyncio import Event, Semaphore, create_task, gather, sleep, Queue, QueueEmpty
...
class ClipboardMonitor(TikTok):
    def __init__(
        ...
        self.queue_dy = Queue()
        self.queue_tk = Queue()
        self._link_semaphore = Semaphore(10)
    ...
    async def check_clipboard(
        ...
        while not self.event_clipboard.is_set():
            if (c := paste()).lower() == "close":
                ...
                create_task(self._check_link_limited(c))
            await sleep(delay)

    async def _check_link_limited(self, text: str):
        async with self._link_semaphore:
