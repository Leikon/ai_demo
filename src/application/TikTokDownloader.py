from asyncio import CancelledError, run, sleep as async_sleep
...
class TikTokDownloader:
    ...
    async def main_menu(
        ...
        while self.running:
            self.__update_menu()
            if not mode:
                mode = safe_pop(self.run_command) or choose(
    ...
    def periodic_update_params(self):
        async def inner():
            while not self.event_cookie.is_set():
                ...
                await async_sleep(COOKIE_UPDATE_INTERVAL)
    ...
    def restart_cycle_task(
        ...
        if restart:
            ...
            self.params_task.join(timeout=COOKIE_UPDATE_INTERVAL + 5)
        self.params_task = Thread(target=self.periodic_update_params, daemon=True)
